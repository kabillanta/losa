  ...
  return (
    <div className="flex flex-col w-full max-w-5xl mx-auto animate-slide-up py-6 lg:py-10 px-4">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
        <div>
          <h1 className="text-3xl md:text-4xl font-semibold text-onyx tracking-tight">
            Admin Dashboard
          </h1>
          <p className="text-taupe mt-2">
            Live event statistics and management.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative group">
            ...
            <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-xl shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
              <a
                href="/api/export/school"
                className="block px-4 py-3 text-sm text-onyx hover:bg-gray-50 font-medium border-b border-gray-100"
              >
                🏫 School Data
              </a>
              <a
                href="/api/export/event"
                className="block px-4 py-3 text-sm text-onyx hover:bg-gray-50 font-medium border-b border-gray-100"
              >
                🎭 Event Data
              </a>
              <a
                ...
                className="block px-4 py-3 text-sm text-onyx hover:bg-gray-50 font-medium"
              >
                📋 Master List
              </a>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col gap-2">
          ...
          <div className="text-4xl font-semibold text-onyx">
            {totalStudents}
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-gold/40 shadow-sm flex flex-col gap-2 relative overflow-hidden">
          ...
          <div className="text-4xl font-semibold text-onyx relative z-10 flex items-baseline gap-2">
            {presentStudents}
            <span className="text-lg text-taupe font-medium">
            ...
            <tbody className="divide-y divide-gray-100">
              {schools?.map((school) => {
                const schoolStudents =
                  students?.filter((s) => s.school_id === school.id) || [];
                const schoolPresent = schoolStudents.filter(
                  (s) => s.is_present,
                ).length;
                const isComplete =
                  schoolStudents.length > 0 &&
                  schoolPresent === schoolStudents.length;
                ...
                return (
                  ...
                  >
                    <td className="p-4 md:p-5 font-medium text-onyx">
                      {school.name}
                    </td>
                    <td className="p-4 md:p-5 text-taupe">
                      {school.teacher_name || "—"}
                    </td>
                    <td className="p-4 md:p-5 text-center tabular-nums">
                      {schoolStudents.length}
                    </td>
                    <td className="p-4 md:p-5 text-center tabular-nums font-medium text-onyx">
                      {schoolPresent}
                    </td>
