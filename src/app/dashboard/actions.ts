...
export async function saveEventEnrollments(
  ...
  for (let teamIndex = 0; teamIndex < teams.length; teamIndex++) {
    ...
    for (const student of validStudents) {
      ...
      if (!existingStudent) {
        ...
        if (createError)
          return { error: "Failed to create student: " + createError.message };
      ...
      if (existingStudent) {
        ...
        if (
          enrollmentsToInsert.some((e) => e.student_id === existingStudent.id)
        ) {
          return {
            error: `Duplicate participant found. ${isTeacherEvent ? "Teacher name" : "Admission number"} '${isTeacherEvent ? student.name.trim() : student.admissionNumber.trim()}' is entered multiple times.`,
          };
        }
  ...
  if (enrollmentsToInsert.length > 0) {
    ...
    if (enrollError)
      return { error: "Failed to save enrollments: " + enrollError.message };
  }
